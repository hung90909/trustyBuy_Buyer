...
const DetailProducts = () => {
    ...
    const [dataComments, setDataComments] = useState([
        {
            id: 1,
            avatarUser: "https://th.bing.com/th?q=Anime+Profile%2fAvatar&w=120&h=120&c=1&rs=1&qlt=90&cb=1&dpr=1.7&pid=InlineBlock&mkt=en-WW&cc=VN&setlang=en&adlt=strict&t=1&mw=247",
        ...
        {
            id: 2,
            avatarUser: "https://th.bing.com/th/id/OIP.e7c0V5z2dSlTZ60O2x_M-wHaHa?w=208&h=209&c=7&r=0&o=5&dpr=1.7&pid=1.7",
        ...
        {
            id: 3,
            avatarUser: "https://th.bing.com/th/id/OIP.aqSg1mTN3zT4TVo2rWuYaQHaHa?w=164&h=180&c=7&r=0&o=5&dpr=1.7&pid=1.7",
            nameUser: "Pham Tien Dung",
            textComment: "Ao rat dao ben, giao hang nhanh gia ca hop ly",
            like: 150,
            star: 2,
    ...
    const renderSanpham = ({ item }) => {
        ...
        return (
            <TouchableOpacity
                onPress={() => nav.replace("DetailProducts", { item })}
                style={{
                    width: '45%',
    ...
    return (
        <SafeAreaView>
            ...
            <ScrollView nestedScrollEnabled={true} style={{ width: "100%", backgroundColor: "white" }}>
                <View style={{ height: "100%", }}>
                    ...
                    <Image
                        resizeMode='stretch'
                        style={{
                            width: "100%", height: 270, zIndex: -999
                        }} source={{ uri: item.imageSP }} />

                    <View style={{ padding: 10 }}>
                        <Text numberOfLines={2} style={styles.nameSP}>{item.nameSP}</Text>
                        ...
                        <View style={{ flexDirection: "row", marginTop: 5, alignItems: "center" }}>
                            ...
                            <Text style={{
                                marginStart: 10, fontSize: 12,
                            }}>Đã bán {formatSoldSP(item.soldSP)}</Text>
                        </View>
                    ...
                    <View style={{ flexDirection: "row", marginHorizontal: 10, marginTop: 15 }}>
                        ...
                        }} source={{ uri: "https://th.bing.com/th/id/OIP.RtHGXHLcBW-bcTn7o76_qAHaHa?w=172&h=180&c=7&r=0&o=5&dpr=1.7&pid=1.7" }} />
                        <View style={{ marginStart: 10 }}>
                            <Text style={{ color: "black", fontWeight: "bold" }}>Puma Việt Nam Official </Text>
                    ...
                    <View style={{ paddingHorizontal: 10, marginTop: 20 }}>
                        <Text style={{ color: "black", fontSize: 16, fontWeight: "bold" }}>Chi tiết sản phẩm</Text>
                        <Text numberOfLines={showDec ? undefined : 3}>Quần Jogger thun Dây Rút SUZY Form Rộng Nam Nữ Unisex 🖤 Quần dài ống rộng suông TÚI HỘP Y2K phong cách Ulzzang
                            Một chiếc quần thun phối màu mới về cực trending cùng chất vải thun cotton có thể mix với nhiều dạng áo khác nhau sẽ tạo cho bạn được đa phong cách trong phối đồ nè 😇😇. Đảm bảo mẫu này cực phù hợp dành cho bạn khi đi học luôn nha, nếu bạn chưa có 1 chiếc quần ống rộng để thay đổi style thì thử e này nha
                            • Màu KEM/ĐEN
                            • 3 size M L XL (40-50kg). Form unisex nam & nữ mặc đều phù hợp nhé.</Text>
                        <TouchableOpacity style={styles.showDec} onPress={() => {
                            handleShowDec()
                        }}>
                            <Text style={{ color: "black" }}>{showDec ? "Thu gọn" : "Xem thêm"}</Text>
                        </TouchableOpacity>
                    </View>
                    <View style={{ paddingHorizontal: 10, }}>
                        <Text style={{ color: "black", fontWeight: "bold", fontSize: 16 }}>Đánh giá sản phẩm</Text>

                        <View style={{ flexDirection: "row", marginTop: 5, alignItems: "center" }}>
                            <FiveStar numberStar={item.star} width={12} height={12} />
                            <Text style={{ marginLeft: 5 }}>{item.star}/5</Text>
                            <Text style={{ marginLeft: 10, fontSize: 11 }}>({formatNumber(1000)} đánh giá)</Text>
                        </View>
                                        ...
                                        <View style={styles.itemComment}>
                                            <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
                                                ...
                                                <View style={styles.cssStar}>
                                                    <Text style={{ color: "black", marginRight: 2 }}>{item.star}</Text>
                                                    <Ionicons name='star' color={"orange"} />
                                                </View>
                                            </View>
                                            <Text style={{ color: "black", marginTop: 6 }}>{item.textComment}</Text>
                                            <View style={{ flexDirection: "row", marginTop: 6 }}>
                                                <View style={{ flexDirection: "row" }}>
                                                    <TouchableOpacity
                                                        onPress={() => handleLikes(item.id)}>
                                                        {likedComment.includes(item.id) ? <Ionicons name='heart' color={"red"} size={20} />
                                                            : <Ionicons name='heart-outline' color={"red"} size={20} />}

                                                    </TouchableOpacity>
                                                    <Text style={{ color: "black", marginLeft: 3 }}>{item.like}</Text>
                                                </View>
                                                <Text style={{ marginLeft: 25 }}>{timeAgo(item.timeComment)} trước</Text>
                                            </View>
                                        </View>
                                    )
                                }}
                    ...
                    <View style={styles.relaiedProduct}>
                        <Text style={styles.textRelaiedProduct}>Gợi ý sản phẩm liên quan</Text>
                        <FlatList
                            scrollEnabled={false}
                            data={sanpham}
                            keyExtractor={item => item.id}
...
const styles = StyleSheet.create({
    ...
    btnBack: {
        position: "absolute", top: 10, zIndex: 999, left: 10
    },
    textRelaiedProduct: {
        marginTop: 10, color: "black", fontSize: 16,
    ...
    btnAll: {
        height: 30, width: 60, backgroundColor: "rgba(217, 217, 217, 1)",
        justifyContent: "center", alignItems: "center",
        borderRadius: 15
    },
    showDec: {
    ...
    btnShowShop: {
        width: 110, height: 27, backgroundColor: "black",
