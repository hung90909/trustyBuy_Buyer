...
import { ADD_CART_API, API_BASE_URL, DISCOUNT_API, GET_ADDRESS_API, PRODUCT_API } from '../../config/urls';
                ...
                renderItem={({ item }) => {
                    return (
                        <TouchableOpacity style={{
                            width: "100%", minHeight: 80, backgroundColor: "white",
                            marginTop: 20, flexDirection: "row", padding: 5,
                            alignItems: "center"
                        }}>
                            <Image style={{
                                width: 100, height: 100,
                                borderRadius: 50, borderWidth: 1
                            }} source={{ uri: 'https://687c-116-96-45-68.ngrok-free.app/' + item.thumb }} />
                            <View style={{width:"60%"}}>
                                <Text style={{
                                    fontWeight: "bold", color: "black",
                                    fontSize: 18
                                }}>{item.discount_name}</Text>
                                <Text style={{ fontSize: 12 }}>Đơn tối thiểu {formatPrice(item.discount_min_order_value)}</Text>
